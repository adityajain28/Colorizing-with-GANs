...
def turingtest( path, count, real_img, fake_img, delay=0):
    ...
    # plt.imshow(img, interpolation='none')
    #fig = plt.figure()
    im = Image.fromarray((fake_img * 255).astype(np.uint8))
    im.save(path + '/Results/'+'Output'+str(count)+'.jpg')
    #fig.savefig(Image.fromarray(imgs[fake_index]),path + './Results/'+'Output'+str(count)+'.jpg') # Use fig. here
    # plt.show()
    # plt.draw()
...
def visualize(train_log_file, test_log_file, window_width, title=''):
    ...
    fig1.savefig('temp.png', dpi=fig1.dpi)
    plt.show()
    plt.draw()
    plt.pause(.01)
